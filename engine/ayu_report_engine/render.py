...
def _headline(value: str) -> str:
    ...
    if len(value) > 6:
        return (
            f'<span class="headline-line" style="display:block">{_escape(value[:-3])}</span>'
            f'<span class="headline-line accent" style="display:block;color:var(--green)">{_escape(value[-3:])}</span>'
        )
...
function drawHero(ctx,y,draw=true){const titleWidth=PNG_REPORT.score?760:W-M*2;ctx.font=`900 66px ${UI}`;const parts=headlineParts(PNG_REPORT.headline);const titleLines=parts.flatMap((part,partIndex)=>lines(ctx,part,titleWidth).map(line=>({line,accent:partIndex>0})));if(draw)titleLines.forEach((item,index)=>{ctx.fillStyle=item.accent?GREEN:TEXT;ctx.fillText(item.line,M,y+index*70)});if(draw&&PNG_REPORT.score){const score=PNG_REPORT.score;ctx.font=`800 58px ${MONO}`;ctx.fillStyle=TEXT;ctx.fillText(score.value,W-M-155,y+12);ctx.font=`700 22px ${MONO}`;ctx.fillStyle=GREEN;ctx.fillText(score.maximum,W-M-58,y+12);const state=[score.status,score.training_type].filter(Boolean).join(' · ');ctx.beginPath();ctx.arc(W-M-145,y+58,5,0,Math.PI*2);ctx.fill();ctx.font=`500 17px ${UI}`;ctx.fillStyle=SECONDARY;ctx.fillText(state,W-M-130,y+64)}let next=y+titleLines.length*70+28;next=textBlock(ctx,PNG_REPORT.subtitle,M,next,titleWidth,`500 22px ${UI}`,34,SECONDARY,draw);if(draw)divider(ctx,next+30);return next+66}
